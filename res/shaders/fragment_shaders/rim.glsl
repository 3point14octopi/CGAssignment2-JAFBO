...
float CalcRimIntensity(vec3 eyeVec, vec3 norm){
...
void main() {
	...
	float rimLightIntensity = CalcRimIntensity(eye, normal);
	vec4 rimLight = vec4((rimLightIntensity *textureColor.xyz), textureColor.w);

	// combine for the final result
	vec3 result = lightAccumulation  * inColor * textureColor.rgb * rimLight.xyz;
